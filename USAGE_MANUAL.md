...
When a source-verification JSON is archived, the verifier also checks report-level source-path parity before `archive_report_source_paths_verified=true`, checks source-report archive-entry path/SHA256/size before `source_report_archive_entry_metadata_verified=true`, checks the manifest-level source-verifier archive entry path/SHA256/size before `source_report_verification_archive_entry_metadata_verified=true`, rejects unreferenced source-verifier/source-report archive records before `source_report_verification_manifest_entry_set_verified=true`, rejects unreferenced rewritten report records before `archive_report_manifest_entry_set_verified=true`, rejects noncanonical fixed rewritten-report or source-verifier member relocation before `archive_report_fixed_paths_verified=true` or `source_report_verification_fixed_paths_verified=true`, treats rewritten report file-entry role/SHA256/size drift as failed `archive_report_metadata_verified`, requires the rewritten report bytes to match a deterministic archive-path rewrite of the archived source report before `archive_report_source_rewrite_verified=true`, fails `archive_inventory_verified` on duplicate, unsafe, symlink, missing, unexpected, or duplicate-manifest archive members, fails `archive_source_paths_verified` on missing, unexpected, non-string, or duplicate `manifest.files[].source_path` identities, treats non-archive `requested_output_file` values as replay-critical path-binding failures, clears `requested_output_file` during archive creation when no correction output member was packaged, then requires the archived JSON to carry its own archive member path plus source-report archive path/SHA256/size metadata before `source_report_verification_archive_metadata_verified=true`.
